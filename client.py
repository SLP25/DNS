...
resolver = (match.group('ip'), int(match.group('port')) if match.group('port') else utils.DEFAULT_PORT)
query = QueryInfo(match.group('d'), EntryType[match.group('t')])
msg = DNSMessage.from_query(query, match.group('r') != None)

server = UDP()
server.send(str(msg).encode(), resolver)    #TODO: normal mode (send as binary)
